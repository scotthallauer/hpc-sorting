...
#SBATCH --nodes=2 --ntasks=4 --ntasks-per-node=2
...
#SBATCH --job-name="MPI Regular Sampling Job (2 Nodes, 4 Tasks)"
...
#SBATCH --output=/home/hpc07/sorting/tests/out_mpi_regsampling_n2t4.txt
#SBATCH --error=/home/hpc07/sorting/tests/err_mpi_regsampling_n2t4.txt
...
do
    java -cp bin Generator bin/numbers_mpi_regsampling_n2t4.txt $j
    for i in {1..3}
    do
        echo "$j [$i]"
        mpiexec -n 4 bin/parallel_mpi_regsampling bin/numbers_mpi_regsampling_n2t4.txt
    done
done

rm bin/numbers_mpi_regsampling_n2t4.txt
