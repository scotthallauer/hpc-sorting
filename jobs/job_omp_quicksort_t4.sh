...
#SBATCH --job-name="OpenMP Quicksort Job (4 Threads)"
...
#SBATCH --output=/home/hpc07/sorting/tests/out_omp_quicksort_t4.txt
#SBATCH --error=/home/hpc07/sorting/tests/err_omp_quicksort_t4.txt
...
do
    java -cp bin Generator bin/numbers_omp_quicksort_t4.txt $j
    for i in {1..3}
    do
        echo "$j [$i]"
        bin/parallel_omp_quicksort bin/numbers_omp_quicksort_t4.txt
    done
done

rm bin/numbers_omp_quicksort_t4.txt
