...
#SBATCH --nodes=1 --ntasks=2 --ntasks-per-node=2
...
#SBATCH --job-name="MPI Regular Sampling Job (1 Node, 2 Tasks)"
...
#SBATCH --output=/home/hpc07/sorting/tests/out_mpi_regsampling_n1t2.txt
#SBATCH --error=/home/hpc07/sorting/tests/err_mpi_regsampling_n1t2.txt
...
do
    java -cp bin Generator bin/numbers_mpi_regsampling_n1t2.txt $j
    for i in {1..3}
    do
        echo "$j [$i]"
        mpiexec -n 2 bin/parallel_mpi_regsampling bin/numbers_mpi_regsampling_n1t2.txt
    done
done

rm bin/numbers_mpi_regsampling_n1t2.txt
